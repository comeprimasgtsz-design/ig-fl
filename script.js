...
// Configuração dos sites - APENAS OS SELETORES FORAM ATUALIZADOS
const sitesConfig = [
...
class MultiSiteAutomation {
    ...
    async initBrowser() {
        // Usando puppeteer-core como no original
        this.browser = await puppeteer.launch({ 
            headless: "new", // Voltando para o padrão comum
    ...
    async login(site) {
        try {
            ...
            // SELETOR ATUALIZADO: Verificação de redirecionamento necessária para sites que mudam a URL de login
    ...
    async sendFollowers(site, target, count) {
        try {
            ...
            // SELETORES DE BUSCA ATUALIZADOS
            await this.page.waitForSelector("input[name='username']", { timeout: 10000 });
            await this.page.type("input[name='username']", target);
            await this.page.click("button[type='submit']");
            
            // SELETOR DE QUANTIDADE ATUALIZADO (Busca dinâmica)
            let qtySel = null;
            for (let i = 0; i < 6; i++) {
                await new Promise(r => setTimeout(r, 5000));
