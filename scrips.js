   ...
   async getFollowerCredits(siteConfig) {
    ...
    async sendFollowers(siteConfig, targetUsername, followerCount = 500) {
        try {
            ...
            // Encontrar o botão "FİND USER" ou similar
            const findUserButton = await this.page.evaluateHandle((buttonText) => {
                const buttons = Array.from(document.querySelectorAll("button"));
                return buttons.find(button => button.textContent.toUpperCase().includes(buttonText));
            ...
            if (findUserButton) {
                ...
                // Tentar pressionar Enter se o botão não for encontrado
                await this.page.keyboard.press('Enter');
                await this.page.waitForNavigation({ waitUntil: "networkidle2", timeout: 10000 }).catch(() => {});
                Logger.info(siteConfig.name, "Enter pressionado para buscar usuário");
            }
            ...
            const startButton = await this.page.evaluateHandle((buttonTexts) => {
                ...
                return buttons.find(button => 
                    buttonTexts.some(text => button.textContent.toUpperCase().includes(text.toUpperCase()))
                );
            ...
            if (startButton) {
                ...
                // Tentar por ID se o texto falhar
                const startBtnById = await this.page.$('#formTakipSubmitButton');
                if (startBtnById) {
                    await startBtnById.click();
                    Logger.success(siteConfig.name, `Processo de envio iniciado via ID!`);
                    return true;
                }
