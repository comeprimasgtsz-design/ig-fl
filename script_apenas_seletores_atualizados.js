...
const sitesConfig = [
    ...
    // Site 'takipcikutusu' teve problemas de login (processamento indefinido).
    // { 
    //     name: "takipcikutusu", 
    ...
    //     findUserButtonText: "Giriş yap", 
    //     startButtonText: ["Giriş yap", "Gönderimi Başlat", "Start"]
    // },
...
class MultiSiteAutomation {
    ...
    async sendFollowers(siteConfig, targetUsername) {
        ...
        await Promise.all([
            this.page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 60000 }),
            this.page.click(siteConfig.findUserButtonText.includes('Find User') || siteConfig.findUserButtonText.includes('FİND USER') || siteConfig.findUserButtonText.includes('Giriş yap') ? 'button[type="submit"]' : `button:contains("${siteConfig.findUserButtonText}")`)
        ]);
        ...
        if (availableCredits > 0) {
            ...
            await Promise.all([
                this.page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 60000 }),
                this.page.click(siteConfig.startButtonText[0].includes('Start') || siteConfig.startButtonText[0].includes('Başlat') || siteConfig.startButtonText[0].includes('Giriş yap') ? 'button[type="submit"]' : `button:contains("${siteConfig.startButtonText[0]}")`)
            ]);
...
// Exemplo de uso (descomente e preencha com suas credenciais para executar)
